...
def ter(inputwords, refwords, lc, term_ids = []):
    ...
    ed = CachedEditDistance(refwords, lc)
    return _ter(inputwords, refwords, ed, term_ids)


def _ter(iwords, rwords, mtd, term_ids = []):
    ...
    # print('[ED]', mtd(iwords))
    while True:
        delta, new_iwords = _shift(iwords, rwords, mtd, term_ids)
        # print('[I]', u' '.join(iwords))
        # print('[R]', u' '.join(rwords))
        # print('[ED]', mtd(iwords))
        if delta <= 0:
...
class CachedEditDistance(object):
    ...
    """
    def __init__(self, rwords, lc):
        self.lc = lc
        self.rwds = rwords
        self._cache = {}
        self.list_for_copy = [0 for _ in range(len(self.rwds) + 1)]

    def __call__(self, iwords, term_ids = []):
        start_position, cached_score = self._find_cache(iwords)
        score, newly_created_matrix = self._edit_distance(iwords, start_position, cached_score, term_ids)
        self._add_cache(iwords, newly_created_matrix)  # もう一度たどって、キャッシュがないノードにキャッシュを挿入していく
        return score

    def _edit_distance(self, iwords, spos, cache, term_ids = []):
        u""" sposが0の場合はキャッシュなし。
        """
        if cache is None:
        ...
        for i, j in itrt.product(range(1, len(iwords) - spos + 1), range(len(self.rwds) + 1)):
            if j == 0:
                ...
                if iwords[spos + i - 1] == self.rwds[j - 1]:
                    cost1 = 0
                else:
                    if (str)(j - 1) in term_ids:
                        cost1 = self.lc
                    else:
                        cost1 = 1
                if (str)(j - 1) in term_ids:
                    ...
                    cost2 = 1
                if (str)(j) in term_ids:
                    cost3 = self.lc
                else:
                    cost3 = 1
                
                l[i][j] = min(l[i - 1][j] + cost3, l[i][j - 1] + cost2, l[i - 1][j - 1] + cost1)
        return l[-1][-1], l[1:]
